...
#include "magic/pkg_io_t.start.hpp"
namespace fx
{
    class device_t;
    class object_t;
    ...
    {                                                                                
        ...
        {                                                                            
            ...
            SDK_MAGIC_PROPERTIES( "FxPkgIo.m_ObjectFlagsByName.$", 0x2, true, 0x3b66d6c7171e6c60 );                                                       
        ...
        _m24 class fx::io_queue_t*                 m_default_queue;                    //{ +0x0078    +0x0078    +0x0078    +0x0078    } | .m_DefaultQueue
        _m25 nt::list_entry_t                      m_io_queue_list_head;               //{ +0x0080    +0x0080    +0x0080    +0x0080    } | .m_IoQueueListHead
        _m26 sdk::array<class fx::io_queue_t*, 28> m_dispatch_table;                   //{ +0x0090    +0x0090    +0x0090    +0x0090    } | .m_DispatchTable
        _m27 uint32_t                              m_random_seed;                      //{ +0x0170    +0x0170    +0x0170    +0x0170    } | .m_RandomSeed
        _m28 uint8_t                               m_filter;                           //{ +0x0174    +0x0174    +0x0174    +0x0174    } | .m_Filter
        _m29 uint8_t                               m_power_state_on;                   //{ +0x0175    +0x0175    +0x0175    +0x0175    } | .m_PowerStateOn
        _m30 uint8_t                               m_queues_are_shutting_down;         //{ +0x0176    +0x0176    +0x0176    +0x0176    } | .m_QueuesAreShuttingDown
        _m31 nt::list_entry_t                      m_dynamic_dispatch_info_list_head;  //{ +0x0178    +0x0178    +0x0178    +0x0178    } | .m_DynamicDispatchInfoListHead
        _m32 class fx::io_in_caller_context_t      m_in_caller_context_callback;       //{ +0x0188    +0x0188    +0x0188    +0x0188    } | .m_InCallerContextCallback
                                                                                     
        SDK_MAGIC_PROPERTIES( "FxPkgIo.$", 0x190, true, 0xa88c34144f613110 );                                  
        SDK_FIXED_SIZE( pkg_io_t, 0x190 );                                           
    };                                                                               
};
#include "magic/pkg_io_t.end.hpp"
SDK_VERIFY( struct fx::pkg_io_t::u0_m_object_flags_by_name_t, 0x2 );
SDK_VERIFY( class fx::pkg_io_t, 0x190 );
