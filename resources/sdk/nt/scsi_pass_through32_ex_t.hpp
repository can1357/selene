...
#include "magic/scsi_pass_through32_ex_t.start.hpp"
namespace nt
{
    // [struct _SCSI_PASS_THROUGH32_EX]
    // => WDK 10 (NV)
    //
    struct scsi_pass_through32_ex_t                          
    {                                                        
        // WDK 10                                            
        //                                                   
        ...
        _m12 uint32_t               data_out_buffer_offset;    //{ +0x0028    } | .DataOutBufferOffset
        _m13 uint32_t               data_in_buffer_offset;     //{ +0x002c    } | .DataInBufferOffset
        _m14 sdk::array<uint8_t, 1> cdb;                       //{ +0x0030    } | .Cdb
                                                             
        SDK_NONVOL_PROPERTIES( "_SCSI_PASS_THROUGH32_EX.$", 0x0, false, 0x6f473c41ece263b1 );                         
        SDK_FIXED_SIZE( scsi_pass_through32_ex_t, 0x34 );                         
    };                                                       
};
#include "magic/scsi_pass_through32_ex_t.end.hpp"
SDK_VERIFY( struct nt::scsi_pass_through32_ex_t, 0x34 );
