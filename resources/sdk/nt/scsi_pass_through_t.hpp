...
#include "magic/scsi_pass_through_t.start.hpp"
namespace nt
{
    // [struct _SCSI_PASS_THROUGH]
    // => WDK 10 (NV)
    //
    struct scsi_pass_through_t                            
    {                                                     
        // WDK 10                                         
        //                                                
        ...
        _m10 uint64_t                data_buffer_offset;    //{ +0x0018    } | .DataBufferOffset
        _m11 uint32_t                sense_info_offset;     //{ +0x0020    } | .SenseInfoOffset
        _m12 sdk::array<uint8_t, 16> cdb;                   //{ +0x0024    } | .Cdb
                                                          
        SDK_NONVOL_PROPERTIES( "_SCSI_PASS_THROUGH.$", 0x0, false, 0xf73dc4c90130e42a );                     
        SDK_FIXED_SIZE( scsi_pass_through_t, 0x38 );                     
    };                                                    
};
#include "magic/scsi_pass_through_t.end.hpp"
SDK_VERIFY( struct nt::scsi_pass_through_t, 0x38 );
