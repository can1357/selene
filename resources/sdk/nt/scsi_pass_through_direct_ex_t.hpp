...
#include "magic/scsi_pass_through_direct_ex_t.start.hpp"
namespace nt
{
    // [struct _SCSI_PASS_THROUGH_DIRECT_EX]
    // => WDK 10 (NV)
    //
    struct scsi_pass_through_direct_ex_t                     
    {                                                        
        // WDK 10                                            
        //                                                   
        ...
        _m12 void*                  data_out_buffer;           //{ +0x0028    } | .DataOutBuffer
        _m13 void*                  data_in_buffer;            //{ +0x0030    } | .DataInBuffer
        _m14 sdk::array<uint8_t, 1> cdb;                       //{ +0x0038    } | .Cdb
                                                             
        SDK_NONVOL_PROPERTIES( "_SCSI_PASS_THROUGH_DIRECT_EX.$", 0x0, false, 0xb856a921c356df8a );                         
        SDK_FIXED_SIZE( scsi_pass_through_direct_ex_t, 0x40 );                         
    };                                                       
};
#include "magic/scsi_pass_through_direct_ex_t.end.hpp"
SDK_VERIFY( struct nt::scsi_pass_through_direct_ex_t, 0x40 );
