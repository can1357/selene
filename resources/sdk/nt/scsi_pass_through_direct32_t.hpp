...
#include "magic/scsi_pass_through_direct32_t.start.hpp"
namespace nt
{
    // [struct _SCSI_PASS_THROUGH_DIRECT32]
    // => WDK 10 (NV)
    //
    struct scsi_pass_through_direct32_t                   
    {                                                     
        // WDK 10                                         
        //                                                
        ...
        _m10 void __ptr32*           data_buffer;           //{ +0x0014    } | .DataBuffer
        _m11 uint32_t                sense_info_offset;     //{ +0x0018    } | .SenseInfoOffset
        _m12 sdk::array<uint8_t, 16> cdb;                   //{ +0x001c    } | .Cdb
                                                          
        SDK_NONVOL_PROPERTIES( "_SCSI_PASS_THROUGH_DIRECT32.$", 0x0, false, 0x555baa15306e96cb );                     
        SDK_FIXED_SIZE( scsi_pass_through_direct32_t, 0x2c );                     
    };                                                    
};
#include "magic/scsi_pass_through_direct32_t.end.hpp"
SDK_VERIFY( struct nt::scsi_pass_through_direct32_t, 0x2c );
