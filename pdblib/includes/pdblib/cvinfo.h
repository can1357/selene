...
typedef enum CV_type_e {
    ...
    CV_UNSIGNED     = 0x02,         // uint32_tegral size values
...
typedef enum CV_special2_e {
    ...
    ///????
    CV_S2_NON_CHAR_I8 = 0x08,
    CV_S2_NON_CHAR_U8 = 0x09,
} CV_special2_e;
...
//      8 bit int types
    ...
    T_UINT1         = 0x0069,   // 8 bit uint32_t
    T_PUINT1        = 0x0169,   // 16 bit pointer to 8 bit uint32_t
    T_PFUINT1       = 0x0269,   // 16:16 far pointer to 8 bit uint32_t
    T_PHUINT1       = 0x0369,   // 16:16 huge pointer to 8 bit uint32_t
    T_32PUINT1      = 0x0469,   // 32 bit pointer to 8 bit uint32_t
    T_32PFUINT1     = 0x0569,   // 16:32 pointer to 8 bit uint32_t
    T_64PUINT1      = 0x0669,   // 64 bit pointer to 8 bit uint32_t
...
//      16 bit int types
    ...
    T_UINT2         = 0x0073,   // 16 bit uint32_t
    T_PUINT2        = 0x0173,   // 16 bit pointer to 16 bit uint32_t
    T_PFUINT2       = 0x0273,   // 16:16 far pointer to 16 bit uint32_t
    T_PHUINT2       = 0x0373,   // 16:16 huge pointer to 16 bit uint32_t
    T_32PUINT2      = 0x0473,   // 32 bit pointer to 16 bit uint32_t
    T_32PFUINT2     = 0x0573,   // 16:32 pointer to 16 bit uint32_t
    T_64PUINT2      = 0x0673,   // 64 bit pointer to 16 bit uint32_t
...
//      32 bit int types
    ...
    T_UINT4         = 0x0075,   // 32 bit uint32_t
    T_PUINT4        = 0x0175,   // 16 bit pointer to 32 bit uint32_t
    T_PFUINT4       = 0x0275,   // 16:16 far pointer to 32 bit uint32_t
    T_PHUINT4       = 0x0375,   // 16:16 huge pointer to 32 bit uint32_t
    T_32PUINT4      = 0x0475,   // 32 bit pointer to 32 bit uint32_t
    T_32PFUINT4     = 0x0575,   // 16:32 pointer to 32 bit uint32_t
    T_64PUINT4      = 0x0675,   // 64 bit pointer to 32 bit uint32_t
...
//      128 bit int types
    ...
    T_UINT16        = 0x0079,   // 128 bit uint32_t
    T_PUINT16       = 0x0179,   // 16 bit pointer to 128 bit uint32_t
    T_PFUINT16      = 0x0279,   // 16:16 far pointer to 128 bit uint32_t
    T_PHUINT16      = 0x0379,   // 16:16 huge pointer to 128 bit uint32_t
    T_32PUINT16     = 0x0479,   // 32 bit pointer to 128 bit uint32_t
    T_32PFUINT16    = 0x0579,   // 16:32 pointer to 128 bit uint32_t
    T_64PUINT16     = 0x0679,   // 64 bit pointer to 128 bit uint32_t
...
typedef struct TYPTYPE {
    uint16_t  len;
    uint16_t  leaf;
    uint8_t   data[CV_ZEROLEN];
} TYPTYPE;          // general types record

__INLINE int8_t*NextType ( int8_t* pType) {
...
typedef struct lfArray_16t {
    ...
    uint8_t   data[CV_ZEROLEN];         // variable length data specifying
...
typedef struct lfClass_16t {
    ...
    uint8_t   data[CV_ZEROLEN];         // data describing length of structure in
                                    // bytes and name
} lfClass_16t;
...
typedef struct lfUnion_16t {
    ...
    uint8_t   data[CV_ZEROLEN];         // variable length data describing length of
                                    // structure and name
} lfUnion_16t;
...
typedef struct lfVTShape {
    ...
    uint8_t   desc[CV_ZEROLEN];     // 4 bit (CV_VTS_desc) descriptors
} lfVTShape;
...
typedef struct lfPreComp_16t {
    ...
    uint8_t   name[CV_ZEROLEN];     // length prefixed name of included type file
} lfPreComp_16t;
...
typedef struct lfTypeServer {
    ...
    uint8_t   name[CV_ZEROLEN];     // length prefixed name of PDB
} lfTypeServer;
...
typedef struct lfSkip_16t {
    ...
    uint8_t   data[CV_ZEROLEN];     // pad data
} lfSkip_16t;
...
typedef struct lfDefArg_16t {
    ...
    uint8_t   expr[CV_ZEROLEN];   // length prefixed expression string
} lfDefArg_16t;
...
typedef struct lfList {
    ...
    int8_t            data[CV_ZEROLEN];         // data format specified by indexing type
} lfList;
...
typedef struct lfFieldList_16t {
    ...
    int8_t            data[CV_ZEROLEN];         // field list sub lists
} lfFieldList_16t;
...
typedef struct lfMethodList_16t {
    uint16_t leaf;
    uint8_t  mList[CV_ZEROLEN];         // really a mlMethod_16t type
} lfMethodList_16t;

typedef struct lfMethodList {
    uint16_t leaf;
    uint8_t  mList[CV_ZEROLEN];         // really a mlMethod type
} lfMethodList;
...
typedef struct lfDimCon_16t {
    ...
    uint8_t   dim[CV_ZEROLEN];          // array of dimension information with
...
typedef struct lfHLSL {
    ...
    uint8_t   data[CV_ZEROLEN];     // variable-length array of numeric properties
...
typedef struct lfModifierEx {
    ...
    uint16_t  mods[CV_ZEROLEN];     // modifiers from CV_modifier_e
} lfModifierEx;
...
typedef struct lfReal128 {
    ...
    int8_t            val[16];        // real 128-bit value
} lfReal128;
...
typedef struct lfCmplx128 {
    ...
    int8_t            val_real[16];   // real component
    int8_t            val_imag[16];   // imaginary component
} lfCmplx128;
...
typedef struct lfVarString {
    ...
    uint8_t   value[CV_ZEROLEN];  // value
} lfVarString;
...
typedef struct lfBClass_16t {
    ...
    uint8_t   offset[CV_ZEROLEN];       // variable length offset of base within class
} lfBClass_16t;
...
typedef struct lfVBClass_16t {
    ...
    CV_fldattr_t    attr;           // attribute
    uint8_t   vbpoff[CV_ZEROLEN];       // virtual base pointer offset from address point
...
typedef struct lfMember_16t {
    ...
    uint8_t   offset[CV_ZEROLEN];       // variable length offset of field followed
...
typedef struct lfEnumerate {
    ...
    uint8_t   value[CV_ZEROLEN];    // variable length value field followed
...
typedef struct CV_DPC_SYM_TAG_MAP_ENTRY {
    uint32_t tagValue;       // address taken symbol's pointer tag value.
...
typedef struct SYMTYPE {
    ...
    int8_t                data[CV_ZEROLEN];
} SYMTYPE;

__INLINE SYMTYPE *NextSym (SYMTYPE * pSym) {
    return (SYMTYPE *) (( int8_t*)pSym + pSym->reclen + sizeof(uint16_t));
}
...
typedef struct ATTRMANYREGSYM {
    ...
    uint8_t   reg[1];     // count register enumerates followed by
                                // length-prefixed name.  Registers are
                                // most significant first.
    uint8_t   name[CV_ZEROLEN];   // utf-8 encoded zero terminate name
} ATTRMANYREGSYM;
...
typedef struct CONSTSYM_16t {
    ...
    uint8_t   name[CV_ZEROLEN];     // Length-prefixed name
} CONSTSYM_16t;
...
typedef struct THUNKSYM16 {
    ...
    uint8_t   variant[CV_ZEROLEN]; // variant portion of thunk
} THUNKSYM16;
...
{
__inline uint16_t ExpandEncodedBasePointerReg(unsigned machineType, unsigned encodedFrameReg) 
{
    static const uint16_t rgFramePointerRegX86[] = {
        CV_REG_NONE, CV_ALLREG_VFRAME, CV_REG_EBP, CV_REG_EBX};
    static const uint16_t rgFramePointerRegX64[] = {
        CV_REG_NONE, CV_AMD64_RSP, CV_AMD64_RBP, CV_AMD64_R13};
    static const uint16_t rgFramePointerRegArm[] = {
...
typedef struct INLINESITESYM {
    ...
    uint8_t   binaryAnnotations[CV_ZEROLEN];   // an array of compressed binary annotations.
} INLINESITESYM;
...
typedef struct LOCALSYM {
    ...
    uint8_t   name[CV_ZEROLEN];   // Name of this symbol, a null terminated array of UTF8 characters.
} LOCALSYM;
...
typedef struct DEFRANGESYMHLSL {    // A live range of variable related to a symbol in HLSL code.
    ...
    CV_LVAR_ADDR_RANGE range;               // Range of addresses where this program is valid
    uint8_t   data[CV_ZEROLEN];       // variable length data specifying gaps where the value is not available
...
typedef struct DISCARDEDSYM {
    ...
    int8_t            data[CV_ZEROLEN];   // Original record(s) with invalid type indices
} DISCARDEDSYM;
...
typedef struct PDBMAP {
    ...
    uint8_t   name[CV_ZEROLEN];   // zero terminated source PDB filename followed by zero
...
typedef uint16_t CV_columnpos_t;    // byte offset in a source line
...
{
    static const uint32_t IndexBitWidth = 20;
    static const uint32_t ImodBitWidth = 12;

    ComboID(uint16_t imod, uint32_t index)
    {
        m_comboID = (((uint32_t) imod) << IndexBitWidth) | index;
    }

    ComboID(uint32_t comboID)
    {
        m_comboID = comboID;
    }
    ...
    uint16_t GetModIndex()
    {
        return (uint16_t) (m_comboID >> IndexBitWidth);
    }
...
{
    static const uint32_t LocalIdBitWidth = 20;
    static const uint32_t IdScopeBitWidth = 11;
    static const uint32_t StartCrossScopeId = 
        (uint32_t) (1 << (LocalIdBitWidth + IdScopeBitWidth));
    static const uint32_t LocalIdMask = (1 << LocalIdBitWidth) - 1;
    static const uint32_t ScopeIdMask = StartCrossScopeId - (1 << LocalIdBitWidth);

    // Compilation unit at most reference 1M constructed type.
    static const uint32_t MaxLocalId = (1 << LocalIdBitWidth) - 1;

    // Compilation unit at most reference to another 2K compilation units.
    static const uint32_t MaxScopeId = (1 << IdScopeBitWidth) - 1;

    CrossScopeId(uint16_t aIdScopeId, uint32_t aLocalId) 
    {  
    ...
    uint32_t GetIdScopeId() {
        return (crossScopeId & ScopeIdMask) >> LocalIdBitWidth;
    }

    static bool IsCrossScopeId(uint32_t i) 
    {
        return (StartCrossScopeId & i) != 0;
    }

    static CrossScopeId Decode(uint32_t i) 
    {
        CrossScopeId retval;
...
typedef struct tagCrossScopeReferences {
    ...
    uint32_t  countOfCrossReferences;     // Count of following array. 
...
inline int32_t BinaryAnnotationInstructionOperandCount(BinaryAnnotationOpcode op)
{
...
//
// Compress an uint32_teger (iLen) and store the result into pDataOut.
//
...
//
// Return value is the uncompressed uint32_teger.  pData is incremented to
